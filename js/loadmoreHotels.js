...
(function($, window, document, undefined) {
    ...
    $.extend(Plugin.prototype, {
        inIt: function() {
            var self = this;
		if(localStorage.getItem('hoteldone')==0 ||localStorage.getItem('hoteldone')==null )
		{$.ajax({
			method: 'GET',
			...
			success: function(d, msg) {
			    self.data.push(d);
			    localStorage.setItem("hoteldetails",JSON.stringify(d));
				if(d != "null" || d != '' || d.length > 0)
				{
				localStorage.setItem('hoteldone','1');
				}
				else{
				localStorage.setItem('hoteldone','0');
				}	
		...
		var d=JSON.parse(localStorage.getItem('hoteldetails'))
		//if(d)
		//{
		self.data.push(d);
		    if (self.data[0].length <= self.options.limit) {
        ...
        More: function() {
            ...
            $(window).scroll(function() {
                ...
                var docH =($('#hoteldetails').children().length)*$('.details').height();
